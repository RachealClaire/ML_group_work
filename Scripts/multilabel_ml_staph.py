...
OUT_DIR   = "/home/kyomukama/Documents/MSBT/Sem2/ML/ML_group_work/Outputs/staph_ml_results"
...
_missing = [v for v in ["X_train_processed","X_test_processed","y_train","y_test"] if v not in _ns]

if _missing:
    raise NameError(
        f"\n[multilabel_ml.py] Missing variables in notebook: {_missing}\n"
        "Define X_train_processed, X_test_processed, y_train, y_test first, then:\n"
        "    %run -i multilabel_ml.py"
    )

_feature_names = _get_names(_ns["X_train_processed"], "feature")
_target_names  = _get_names(_ns["y_train"], "antibiotic")

_X_train = _to_numpy(_ns["X_train_processed"])
_X_test  = _to_numpy(_ns["X_test_processed"])
